...
from .forms import CampusForm, VenueForm, SportEventForm, MeetForm, UserRoleForm
from .permissions import ROLE_ADMIN, ROLE_TEACHER, role_required
from .models import (
    Campus,
    Venue,
    SportEvent,
...
def role_manage(request):
    ...
    if request.method == "POST" and form.is_valid():
        ...
        messages.success(request, f"已将 {user.username} 设置为 {dict(UserProfile.ROLE_CHOICES)[role]}")
        return redirect("role_manage")

    items = UserProfile.objects.select_related("user").order_by("user__username")
    return render(request, "core/role_manage.html", {"items": items, "form": form})
